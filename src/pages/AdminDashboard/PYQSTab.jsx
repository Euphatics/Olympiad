...
export default function PYQSTab() {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      alert(err.message || 'Upload failed. You can paste a PDF direct link instead.');
    } finally {
      setUploading(false);
      e.target.value = '';
    }
  ...
  const handleAdd = async (e) => {
    e.preventDefault();
    if (!newPyq.paperUrl) {
      alert('Please upload a PDF or enter a valid PDF URL first.');
      return;
    }
    setSaving(true);
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this previous question paper entry?')) return;
                    ...
                    pyqs.map(p => (
                      <tr key={p.id} className="hover:bg-gray-50/80 transition-colors">
                        ...
                        <td className="px-5 py-4 text-right border-l border-gray-200">
                          <button onClick={() => handleDelete(p.id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-sm transition-colors" title="Delete paper">
